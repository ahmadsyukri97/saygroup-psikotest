...
function buildFullReportHtml(record) {
  ...
  pages.push(`<div class="report-page"><div class="flex-1" style="font-size:12px;line-height:1.6;color:#3A362F;">
    <h1 class="text-2xl font-bold mb-6 serif" style="color:${INK}">Pengantar Laporan</h1>
    <h3 class="font-bold mb-1" style="color:${INK}">Tujuan Laporan</h3>
    <p class="mb-4">Laporan ini disusun berdasarkan hasil pelaksanaan asesmen psikologi yang dilakukan oleh SAYGROUP Talent Assessment. Laporan bertujuan memberikan gambaran objektif mengenai profil psikologis kandidat berdasarkan alat ukur yang digunakan, sebagai salah satu bahan pertimbangan dalam proses seleksi, penempatan, maupun pengembangan individu.</p>
  ...
  // Kemampuan Umum (IQ) = rata-rata 9 subtes IST yang diisi manual di tab IST,
  // BUKAN record.iq (itu skor kuis screening 60-soal, skala & makna berbeda).
  const istFilledScores = istPts.map((p) => p.value).filter((v) => v > 0);
  const istIq = istFilledScores.length ? Math.round(istFilledScores.reduce((a, v) => a + v, 0) / istFilledScores.length) : 0;
  const istIqCategory = istBand(istIq).label;
  pages.push(`<div class="report-page"><div class="flex-1">
    ...
    <table>
      ...
      <tbody>
        <tr style="background:#F0F1F7;"><td class="px-3 py-2 font-bold" style="color:${INK}">Kemampuan Umum (IQ)</td><td class="px-3 py-2 font-bold" style="color:${INK}">${istIq}</td><td class="px-3 py-2" style="color:${INK}">${escapeHtml(istIqCategory)}</td><td class="px-3 py-2">${scaleBarHtml(istScalePos(istIq))}</td></tr>
        ${istRows}
      </tbody>
    </table>
